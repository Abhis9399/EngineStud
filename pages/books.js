...
export async function getServerSideProps(context) {
  ...
  let products = [];
  products = await product.find({ category: 'Books' }).maxTimeMS(60000); // 60 seconds timeout
  // Logic for books availability and sizes
  let books = {};

  for (let item of products) {
    if (item.title in books) {
      if (!books[item.title].color.includes(item.color) && item.availableQty > 0) {
...
//https://m.media-amazon.com/images/I/61CD8zixp0L._SL1376_.jpg